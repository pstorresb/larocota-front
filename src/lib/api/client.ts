...
export const api = {
  ...
  requestPasswordReset: (input: { email: string }) => request<{ message: string }>("/auth/password-reset/request", { method: "POST", body: JSON.stringify(input) }),
  verifyPasswordReset: (input: { email: string; code: string; password: string }) => request<{ user: AuthUser }>("/auth/password-reset/verify", { method: "POST", body: JSON.stringify(input) }),
